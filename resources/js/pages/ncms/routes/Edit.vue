...
const currentUrl = `/ncms/globalvars/${globalvar.id}/edit`;
...
const { isFieldDirty, handleSubmit, setErrors } = useForm({
    ...
    initialValues: {
        ...
        is_read_only: globalvar.is_read_only,
        is_protected: globalvar.is_protected,
    },
});

const onSubmit = handleSubmit(function (values, actions) {
    axios
        .post(currentUrl, values)
        .then(function (response) {
            if (response.status === 200) {
                toast({
                    title: 'Success',
                    variant: 'default',
                    description: 'Global variable saved.',
                });
                return;
            }
...
function handleDelete() {
    axios
        .delete(`/ncms/globalvars/${globalvar.id}`)
        .then(function (response) {
            if (response.status === 200) {
                toast({
                    title: 'Success',
