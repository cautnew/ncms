...
class CrawlerFundamentusFII extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:crawler-fundamentus-fii';
    ...
    private string $urlToData = 'https://www.fundamentus.com.br/fii_resultado.php';
    ...
    public function handle(): void
    {
        try {
            ...
            $rows = $this->getCrawler()->filter('#tabelaResultado tbody tr');
            $names = $this->getCrawler()->filter('#tabelaResultado tbody tr td span')->extract(['title']);
    ...
    private function clearDataForToday(): void
    {
        ...
        CotacaoFundamentusFII::whereBetween('created_at', [$nowStart, $nowEnd])->delete();
    ...
    private function prepareData($rows, $names): array
    {
        ...
        $cotFIIFundamentus = new CotacaoFundamentusFII();
        $columns = $cotFIIFundamentus->getFillable();
        $dataToInsert = [];
    ...
    private function insertData(array $data): void
    {
        ...
        if (!empty($data)) {
            CotacaoFundamentusFII::insert($data);
